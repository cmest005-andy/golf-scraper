...
def draft_room(request, pk):
    ...
    _rank_expr = Coalesce('player__world_ranking', Value(9999, output_field=IntegerField()))
    available = (
        Leaderboard.objects
        .filter(tournament=draft.tournament)
        .exclude(player_id__in=taken_ids)
        .select_related('player')
        .order_by('total_score_to_par' if has_round_scores else _rank_expr, _rank_expr)
    )
